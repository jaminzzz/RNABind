...
from LucaOneApp.algorithms.args import Args
from LucaOneApp.algorithms.file_operator import fasta_reader, csv_reader
from LucaOneApp.algorithms.utils import set_seed, to_device, get_labels, get_parameter_number, \
    gene_seq_replace, clean_seq, available_gpu_id, download_trained_checkpoint_lucaone, calc_emb_filename_by_seq_id
from LucaOneApp.algorithms.batch_converter import BatchConverter
from LucaOneApp.algorithms.llm.lucagplm.v2_0.lucaone_gplm import LucaGPLM as LucaGPLMV2_0
from LucaOneApp.algorithms.llm.lucagplm.v2_0.lucaone_gplm_config import LucaGPLMConfig as LucaGPLMConfigV2_0
from LucaOneApp.algorithms.llm.lucagplm.v2_0.alphabet import Alphabet as AlphabetV2_0
...
def main(model_args):
    ...
    if model_args.input_file:
        ...
        if seq_type == "prot":
            ...
            embedding = emb.hidden_states
        if model_args.matrix_add_special_token:
            embedding = embedding[0, 0: processed_seq_len, :].to(device="cpu").clone().numpy()
        else:
            embedding = embedding[0, prepend_len: processed_seq_len - append_len, :].to(device="cpu").clone().numpy()
